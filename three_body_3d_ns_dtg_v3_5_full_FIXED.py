#!/usr/bin/env python3
# three_body_3d_ns_dtg_v3_5_full_FIXED.py
# 3D Three-body + drag(ν) + (opt) turbulence + DTG + central anchor potential
# - Backends: Numba -> NumPy fallback, (opt) external C ext
# - DTG: signed/rectified, theta_init, energy/lyap feedback, resonance
# - Integrators: DOP853 (default), --use_symplectic (K-D-K Verlet + dissipation split)
# - Potentials: Newton + Central(Harmonic + Plummer) + (opt) Yukawa(repulsive/screened)
# - Outputs: CSV(main/diagnostics), figures, meta.json, Judgment(Partial/Success)
...
def yukawa_pair_force_np(dr, r, m_j, k, lam, repulsive=True):
    # V = sgn*k*exp(-lam r)/r; |F| = sgn*k*exp(-lam r)*(1/r^2 + lam/r)
    if k == 0.0:
        return 0.0
    sgn = +1.0 if repulsive else -1.0
    fac = sgn * k * np.exp(-lam * r) * (1.0/(r*r) + lam/r) / r  # unit vector mult
    return (fac * m_j) * dr
...
def accelerations_np_full(pos, vel, G, masses, eps=DEFAULT_EPS,
                          ...
                          k_planar=0.0):
    """Total forces (grav + visc + control + turb + center + Yukawa + planar spring) -> accelerations"""
    N = pos.shape[1]
    ...
    F_planar = np.vstack([np.zeros(pos.shape[1]), np.zeros(pos.shape[1]), -k_planar * pos[2, :]])

    return grav + F_yuk + visc + turb + ctrl + F_center + F_planar

def accelerations_np_conservative(pos, vel, G, masses, eps=DEFAULT_EPS,
                                  center_k=0.0, center_GM=0.0, center_eps=0.1,
                                  yukawa_k=0.0, yukawa_lambda=0.5, yukawa_repulsive=True,
                                  k_planar=0.0):
    """Conservative forces only (for symplectic path) + planar spring"""
    N = pos.shape[1]
    dr, r2 = pairwise_dr(pos)
    r2 = r2 + eps*eps
    mask = ~np.eye(N, dtype=bool)
    ...
    F_center = center_force_np(pos, masses, center_k=center_k, center_GM=center_GM, center_eps=center_eps)
    F_planar = np.vstack([np.zeros(pos.shape[1]), np.zeros(pos.shape[1]), -k_planar * pos[2, :]])
    return grav + F_yuk + F_center + F_planar
...
@njit(cache=True, fastmath=True)
def accelerations_numba_full(
    pos, vel, G, masses, eps,
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            invr3 = 1.0 / (r2 * r)
            sG = -G * masses[j] * invr3
            ax += sG*dx; ay += sG*dy; az += sG*dz

            if yukawa_k != 0.0:
                sgn = 1.0 if yukawa_repulsive else -1.0
                fac = sgn * yukawa_k * ((1.0/(r*r) + yukawa_lambda/r) / r)
                fac *= np.exp(-yukawa_lambda * r) * masses[j]
        ...
        if ctrl_signed:
            ...
            td = theta_dev if theta_dev > 0.0 else 0.0
        ...
        if turb_coeff != 0.0:
            ...
            tb = turb_coeff * np.sin(s2)
            ax += tb*vel[0,i]; ay += tb*vel[1,i]; az += tb*vel[2,i]
...
@njit(cache=True, fastmath=True)
def accelerations_numba_conservative(
    pos, vel, G, masses, eps,
...
def rhs_body_conservative(t, s_body, args_rhs):
    ...
    cK = args_rhs.get("center_k",0.0)
    cGM = args_rhs.get("center_GM",0.0)
    cEps = args_rhs.get("center_eps",0.1)
    yk = args_rhs.get("yukawa_k", 0.0)
    yl = args_rhs.get("yukawa_lambda", 0.5)
    yr = args_rhs.get("yukawa_repulsive", True)
    kpl = args_rhs.get("k_planar", 0.0)
    pos, vel = unpack_state_bodies(s_body, N=3)
    acc = _acc_cons(pos, vel, G, masses, eps, cK, cGM, cEps, yk, yl, yr, kpl)
...
class RHSParams:
    def __init__(self, G, masses, eps,
        ...
        self.k_planar = float(k_planar)

def rhs_with_dtg(t, s_all, prm):
    """Full RHS with DTG variable theta as last component.
    Returns d/dt of [state_bodies(=pos,vel), theta].
    """
    pos, vel, theta = split_state_with_theta(s_all, N=3)

    # Energy deviation ed (only if used)
    use_ed = (prm.E0 is not None) and (prm.alpha_dtg != 0.0 or prm.k_nu_ed != 0.0 or prm.k_ctrl_e != 0.0)
    if use_ed:
        E = total_energy_body(
            pack_state_bodies(pos, vel), prm.G, prm.masses, eps=prm.eps,
    ...
    nu_eff = np.clip(prm.nu0 + prm.k_nu*theta_dev + prm.k_nu_ed*ed_eff, 0.0, prm.nu_max)

    k_ctrl_eff = prm.k_ctrl * (1.0 + prm.k_ctrl_e*abs(ed_eff))
    ...
    g = 1.0/(1.0 + np.exp(-(rmin_now - r_mid)/w))
    k_ctrl_eff *= prm.gate_floor + (1.0 - prm.gate_floor) * (1.0 - g)

    # accelerations (full)
    acc = _acc_full(
        pos, vel, prm.G, prm.masses, prm.eps,
...
def dissipative_kick_inplace(pos, vel, dt, nu_eff, k_ctrl_eff, theta_dev, turb_coeff, ctrl_signed):
    # per-body damping factor: nu + k_ctrl*td  (turbulence multiplicative ~)
    r2 = np.sum(pos*pos, axis=0)
    td = theta_dev if ctrl_signed else np.maximum(theta_dev, 0.0)
    damp = (nu_eff + k_ctrl_eff*td)
    if turb_coeff != 0.0:
        damp -= turb_coeff * np.sin(r2)
    factor = (1.0 - 0.5*dt*damp)
    vel *= factor if np.ndim(factor) == 0 else factor[None, :]

def symplectic_step(pos, vel, dt, prm):
    ...
    acc = _acc_cons(pos, vel, prm.G, prm.masses, prm.eps,
                    ...
                    prm.yukawa_k, prm.yukawa_lambda, prm.yukawa_repulsive, prm.k_planar)
    vel += 0.5*dt*acc
    # drift
    pos += dt*vel
    # conservative half-kick
...
def hold_true(x_bool, win):
    ...
    c = np.convolve(x.astype(int), k, mode='same')
    return bool(np.any(c >= w))
...
def run_simulation(args):
    ...
    if args.use_symplectic:
        # symplectic: evaluate final state only
...
def analyze_results(args, meta, csv_diag, lam, drift, theta_hist, planar, fig8_phase):
    """Summarize a single run and print a compact judgment line.
    Robust to missing/short traces.
    """
    # 1) Energy drift
    drift_arr = np.atleast_1d(drift)
    max_drift = float(np.max(np.abs(drift_arr))) if drift_arr.size else float("inf")
    drift_good = (max_drift < getattr(args, "drift_threshold", 0.05))

    # 2) Theta activity (rad/s), prefer CSV on disk; fall back to arrays in memory
    theta_rate = 0.0
    try:
        df_diag = pd.read_csv(meta["outputs"]["csv_diagnostics"])
        tvals = df_diag["t"].to_numpy()
        thvals = df_diag["theta"].to_numpy()
        dt_samp = float(np.mean(np.diff(tvals))) if tvals.size > 1 else float(getattr(args, "dt", 0.002))
        theta_rate = float(np.mean(np.abs(np.diff(thvals))) / max(dt_samp, 1e-12)) if thvals.size > 1 else 0.0
    except Exception:
        T_like = None
        try:
            # If caller passed back the time vector inside csv_diag path, read it
            if isinstance(csv_diag, str) and os.path.exists(csv_diag):
                df_fallback = pd.read_csv(csv_diag)
                T_like = df_fallback["t"].to_numpy()
                th_like = df_fallback["theta"].to_numpy()
                dt_samp = float(np.mean(np.diff(T_like))) if T_like.size > 1 else float(getattr(args, "dt", 0.002))
                theta_rate = float(np.mean(np.abs(np.diff(th_like))) / max(dt_samp, 1e-12)) if th_like.size > 1 else 0.0
        except Exception:
            pass
        if T_like is None:
            dt_samp = float(getattr(args, "dt", 0.002))
            theta_rate = float(np.mean(np.abs(np.diff(theta_hist))) / max(dt_samp, 1e-12)) if len(theta_hist) > 1 else 0.0
    theta_active = bool(theta_rate > 0.01)

    # 3) Orbit qualitative checks
    orbit_ok = bool(planar) or (float(fig8_phase) < 1.0)

    # 4) Lyapunov summary (top exponent)
    lam_top = float(lam[0]) if (lam and len(lam) > 0 and lam[0] is not None) else 0.0
    lam_good = (lam_top < 0.5) if lam else False

    score = int(drift_good) + int(theta_active) + int(orbit_ok) + int(lam_good)
    verdict = "Success" if score >= 3 else ("Partial Success" if score >= 2 else "Failure")

    print(
        f"[Judgment] {verdict} (met {score}/4)"
        f" | drift_max={max_drift:.3g}"
        f" | theta_rate={theta_rate:.4f} rad/s ({theta_active})"
        f" | planar={bool(planar)}, fig8_phase={float(fig8_phase):.3f}"
...
def compare_runs(args, csv_b, csv_d, meta_b, meta_d):
    """Compare baseline vs DTG-enhanced runs using their diagnostics CSVs and meta.
    Returns a small dict with improvement percentages and a PASS/NO-GAIN verdict.
    """
    b = pd.read_csv(csv_b); d = pd.read_csv(csv_d)

    # Energy drift improvement (lower is better)
    b_max = b["drift"].abs().max() if len(b) else 0.0
    d_max = d["drift"].abs().max() if len(d) else 0.0
    imp_drift = 100.0 * (b_max - d_max) / max(1e-12, b_max)

    # Escape/collision improvements (1 if avoided)
    esc_b = 0 if meta_b.get("events", {}).get("escaped", False) else 1
    esc_d = 0 if meta_d.get("events", {}).get("escaped", False) else 1
    imp_escape = 100.0 * (esc_d - esc_b)

    col_b = 0 if meta_b.get("events", {}).get("collided", False) else 1
    col_d = 0 if meta_d.get("events", {}).get("collided", False) else 1
    imp_collision = 100.0 * (col_d - col_b)

    lam_b = meta_b.get("lyap_top") or ((meta_b.get("lyap_spectrum") or [None])[0])
    lam_d = meta_d.get("lyap_top") or ((meta_d.get("lyap_spectrum") or [None])[0])
    imp_lyap = 100.0 * (lam_b - lam_d) / max(1e-12, lam_b) if (lam_b not in (None, 0)) else 0.0

    # Theta activity from DTG run
    dtheta = np.diff(d.get("theta", pd.Series(dtype=float)).to_numpy())
    dt_samp = float(np.mean(np.diff(d["t"].to_numpy()))) if len(d) > 1 else float(getattr(args, "dt", 0.002))
    theta_rate = float(np.mean(np.abs(dtheta)) / max(dt_samp, 1e-12)) if dtheta.size else 0.0
    theta_active = bool(theta_rate > 0.01)
...
def main():
    ...
    ap.add_argument("--recenter_mode", choices=["none","momentum","full"], default="none", help="COM recenter mode")
    ap.add_argument("--recenter_period", type=float, default=60.0, help="COM recenter period [s]")
    args = ap.parse_args()
