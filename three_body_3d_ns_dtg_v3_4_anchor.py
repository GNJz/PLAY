...
# - Numba hot-kernel (pairwise gravity/controls & rmin) 최적화
# - DTG: signed/rectified 제어 선택, θ 초기값 인자 추가
# - Energy accounting includes central anchor (harmonic + Plummer)
...
def accelerations_np(pos, vel, G, masses, eps=DEFAULT_EPS,
                     nu_eff=0.0, turb_coeff=0.0, k_ctrl_eff=0.0, theta_dev=0.0,
                     center_k=0.0, center_GM=0.0, center_eps=0.1, ctrl_signed=False):
    """벡터화 폴백(Numba 미사용시)"""
    N = pos.shape[1]
...
def total_energy_body(s_body, G=1.0, masses=(1,1,1), eps=DEFAULT_EPS,
    ...
    # Center potential (harmonic + Plummer)
...
@njit(cache=True, fastmath=True)
def accelerations_numba(pos, vel, G, masses, eps,
...
# 폴백 헬퍼
def min_pair_distance_np(pos):
    _, r2 = pairwise_dr(pos)
    N = r2.shape[0]
    m = r2 + np.eye(N)*1e18
    return float(np.sqrt(np.min(m)))

MIN_DIST = min_pair_distance_numba if HAVE_NUMBA else min_pair_distance_np
ACCEL = accelerations_numba if HAVE_NUMBA else accelerations_np
...
def rhs_body_conservative(t, s_body, args_rhs):
    ...
    pos, vel = unpack_state_bodies(s_body, N=3)
    # 보수역학이므로 제어/점성=0
    acc = ACCEL(pos, vel, G, masses, eps,
                0.0, 0.0, 0.0, 0.0,
                cK, cGM, cEps, False)
...
def run_simulation(args):
    ...
    def event_collision(t, s):
        pos, _, _ = split_state_with_theta(s, N=3)
        return MIN_DIST(pos) - args.r_collide
