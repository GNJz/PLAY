...
def analyze_results(args, meta, csv_diag, lam, drift, theta_hist, planar, fig8_phase):
    ...
    - Robust to missing/short traces and missing Lyapunov.
    - Thresholds are pulled from args when present, else sensible defaults.
    """
    # -----

# === Drift metrics + robust compare (appended) ===

def _drift_score_from_csv(csv_path):
    """Return {max, rms, auc} of |drift(t)| using time-weighted metrics.
    - max : max |drift|
    - rms : sqrt( ∫|d|^2 dt / ∫dt )
    - auc : ∫|d| dt / (T)
    """
    import numpy as _np
    df = pd.read_csv(csv_path, usecols=["t","drift"])
    t = df["t"].to_numpy()
    d = _np.abs(df["drift"].to_numpy())
    if t.size < 2:
        mx = float(_np.max(d)) if d.size else 0.0
        return dict(max=mx, rms=mx, auc=mx)
    w = _np.diff(t)
    rms = float(_np.sqrt(((d[:-1]**2) * w).sum() / max(w.sum(), 1e-12)))
    auc = float(((d[:-1]) * w).sum() / max(t[-1]-t[0], 1e-12))
    return dict(max=float(_np.max(d)), rms=rms, auc=auc)


def _improve_pct(base, test, eps=1e-6, cap=200.0):
    """(base - test)/base * 100 with safety for tiny denominators, clamped to ±cap."""
    den = max(float(base), eps)
    val = 100.0 * (float(base) - float(test)) / den
    if not np.isfinite(val):
        val = 0.0
    return max(min(val, cap), -cap)


def compare_runs(args, csv_b, csv_d, meta_b, meta_d):
    """Robust single-run comparison using RMS drift (default) and binary improvements
    for escape/collision and Lyapunov (if available).
    Returns a dict with fields used by the scan printer.
    """
    # ---- drift
    metric = getattr(args, "drift_metric", "rms")  # "rms"|"auc"|"max"
    b = _drift_score_from_csv(csv_b)
    d = _drift_score_from_csv(csv_d)
    drift_improve = _improve_pct(b.get(metric, b["rms"]), d.get(metric, d["rms"]))

    # ---- events (binary improvements)
    esc_b = bool(meta_b.get("events", {}).get("escaped", False))
    esc_d = bool(meta_d.get("events", {}).get("escaped", False))
    col_b = bool(meta_b.get("events", {}).get("collided", False))
    col_d = bool(meta_d.get("events", {}).get("collided", False))

    def _bin_improve(a_b, a_d):
        if a_b and not a_d: return +100.0
        if (not a_b) and a_d: return -100.0
        return 0.0

    escape_improve = _bin_improve(esc_b, esc_d)
    collision_improve = _bin_improve(col_b, col_d)

    # ---- Lyapunov (top) if present (light is conservative-only => often identical)
    def _top_lyap(meta):
        try:
            if meta.get("lyap_spectrum"):
                v = float(meta["lyap_spectrum"][0])
    ...
    lb = _top_lyap(meta_b)
    ld = _top_lyap(meta_d)
    lyap_improve = _improve_pct(lb, ld) if (lb is not None and ld is not None) else 0.0

    # ---- theta activity (from DTG run)
    theta_thr = getattr(args, "theta_activity_thresh", 0.01)
    theta_active = bool(meta_d.get("end_state", {}).get("theta_activity", 0.0) > theta_thr)

    # ---- verdict
    need = getattr(args, "pass_drift_improve", 5.0)  # need ≥5% drift improvement by default
    ok = (drift_improve >= need) and (escape_improve >= 0.0) and (collision_improve >= 0.0)
    verdict = "PASS" if ok else "NO-GAIN"

    return dict(
        verdict=verdict,
        drift_improve=drift_improve,
        escape_improve=escape_improve,
        collision_improve=collision_improve,
        lyap_improve=lyap_improve,
