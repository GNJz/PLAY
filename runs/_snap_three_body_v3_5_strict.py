...
# - Hot kernels: Numba -> NumPy fallback, (opt) external C ext
...
import pandas as pd
def _solve_ivp(*a, **k):
    from scipy.integrate import solve_ivp as _sv
    return _sv(*a, **k)
...
def accelerations_np_full(pos, vel, G, masses, eps=DEFAULT_EPS,
                          ...
                          k_planar=0.0):
    """Total forces (grav + visc + control + turb + center + Yukawa + planar spring)"""
    N = pos.shape[1]
...
def lyap_spectrum_benettin(s0_body, rhs_body, tmax=12.0, dt=0.002, k=3, args_rhs=None, seed=0):
    ...
    while t < tmax - 1e-12:
        ...
        sol = _solve_ivp(lambda tt, ss: rhs_body(tt, ss, args_rhs),
                        (t, t_next), s, method="DOP853", rtol=1e-9, atol=1e-12, max_step=dt)
        s = sol.y[:, -1]
        eps_fd = 1e-8
        Z = np.zeros_like(Q)
        for j in range(k):
            sol2 = _solve_ivp(lambda tt, ss: rhs_body(tt, ss, args_rhs),
...
def benettin_light_max(s0_body, rhs_body, dt=0.01, renorm_every=50, tmax=12.0, args_rhs=None, seed=0):
    ...
    while t < tmax - 1e-12:
        ...
        sol1 = _solve_ivp(lambda tt, ss: rhs_body(tt, ss, args_rhs),
                         (t, t_next), s, method="DOP853", rtol=1e-9, atol=1e-12, max_step=dt)
        sol2 = _solve_ivp(lambda tt, ss: rhs_body(tt, ss, args_rhs),
...
def rhs_with_dtg(t, s_all, prm):
    ...
    # Energy deviation (optional)
    use_ed = (prm.E0 is not None) and (prm.alpha_dtg != 0.0 or prm.k_nu_ed != 0.0)
    if use_ed:
        E = total_energy_body(
...
def symplectic_step(pos, vel, dt, prm):
    ...
    dissipative_kick_inplace(pos, vel, dt, prm.nu0, prm.k_ctrl,
                             0.0,
...
def integrate_with_optional_adaptive(rhs, t0, t1, y0, args, prm, events):
    ...
    while cur_t < t1 - 1e-12:
        ...
        sol = _solve_ivp(lambda tt, ss: rhs(tt, ss, prm),
...
def analyze_results(args, meta, csv_diag, lam, drift, theta_hist, planar, fig8_phase):
    ...
    theta_active = theta_rate > args.theta_active_thresh
...
def main():
    ...
    ap.add_argument("--theta_active_thresh", type=float, default=0.01,
                    help="threshold for theta activity (rad/s) in final verdict")
