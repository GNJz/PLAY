#!/usr/bin/env python3
# three_body_3d_ns_dtg_v3_3_stable.py
# 3D Three-body + drag(ν) + (opt) turbulence + DTG
# Merged from v3.2 (stable infra) + v3.3 (adaptive-step, fig8-phase, λ+ score)
# - θ target: theta0 + α*ed − β*λ
# - Lyapunov: light (top-1) or spectrum (top-k), plus λ+ score = sum(max(λ_i,0))
# - Adaptive outer loop with max_step shrinking near close encounters (optional)
# - Vectorized energy & distances, all-pair binary detection
# - Diagnostics: planarity, figure-8 (phase-based), θ-activity
# - Compare mode (--compare, --nu_scan), success judgement
# - Plots toggle (--no_plots)
...
def integrate_with_optional_adaptive(rhs, t0, t1, y0, args, prm, events):
    """Outer loop chunking + optional adaptive max_step."""
    cur_t = float(t0)
    cur_y = y0.copy()
...
def run_simulation(args):
    ...
    if args.adaptive_step:
        ...
        # uniform interpolation to t_eval
...
def analyze_results(args, meta, csv_diag, lam, drift, theta_hist, planar, fig8_phase):
    ...
    theta_act = float(np.mean(np.abs(np.diff(theta_hist))))
    theta_active = theta_act > 0.01
    ...
    print(f"[Judgment] {verdict} (met {score}/4)"
          ...
          f" | theta_act={theta_act:.4f}({theta_active})"
...
def compare_runs(args, csv_b, csv_d, meta_b, meta_d):
    ...
    theta_active = np.mean(np.abs(dtheta)) > 0.01
