#!/usr/bin/env python3
# three_body_3d_ns_dtg_v3_2_fixed.py
# 3체(3D) + 항력(ν) + (옵션) 난류 + DTG(연속시간)
# v3.2 안정판 (피드백 반영)
# - compute_diagnostics 전역화 + 브로드캐스팅 안정화
# - analyze_results 드리프트 판정 = 절대 임계치 비교
# - f-string 조건부 포맷 버그 수정
# - Lyapunov 선택 로직 헬퍼(_pick_lam0)로 일관성 개선
# - provenance 정리
...
def hold_true(x_bool, win):
    # 느슨한 윈도우 판정(가장자리 반창 포함). 엄밀한 연속 판정은 'valid' 컨볼루션 또는 러닝카운트 사용.
...
def main():
    ...
    if args.compare:
        results = []
        # Baseline (dtg=0) & DTG(=1) for each nu0 in scan (if empty, default to current nu0)
