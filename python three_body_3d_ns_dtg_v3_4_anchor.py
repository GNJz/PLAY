...
# ---------- central-force helpers ----------
def center_force(pos, masses, center_k=0.0, center_GM=0.0, center_eps=0.1):
    """Force from central (origin) potential on each body, shape (3,N).
    Harmonic: F_h = -m_i * k * r_i
    Plummer : F_p = -(GM * m_i) * r_i / (|r_i|^2 + eps^2)^{3/2}
    center_GM = G*M_center
    """
...
def accelerations(pos, vel, G, masses, eps=DEFAULT_EPS,
    ...
    return grav + visc + turb + ctrl + (F_center if isinstance(F_center, np.ndarray) else 0.0)
...
def rhs_with_dtg(t, s_all, prm: RHSParams):
    ...
    # close-approach gating for control
...
def run_simulation(args):
    ...
    def compute_diagnostics(bodyY, masses, G, eps, center_k, center_GM, center_eps):
        ...
        speed2 = np.sum(vel**2, axis=0)
        K = 0.5 * np.sum(masses[:, None] * speed2, axis=0)
        ...
        U_pairs = -G * np.sum(pair_mass / r[iu], axis=0)
        r2_self = np.sum(pos*pos, axis=0)
