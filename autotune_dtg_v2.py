...
"""
autotune_dtg_v2.py
- DTG 컨트롤러 파라미터 자동 탐색/검증 오케스트레이터
- 요구 스크립트: three_body_3d_ns_dtg_v3_3_stable.py  (동일 디렉터리 가정)

기능
- explore : 초기 균등 샘플 + 지역 변이 탐색
- exploit : 상위 후보를 장주행(tmax_long)으로 재검증
- summarize : 누적 로그 정렬/상위 k 출력

안정성
- 고정 CSV 스키마(FIELDNAMES) 사용
- append/load/summarize 모두 견고화
"""
...
def read_params_from_meta(meta_dir: Path) -> Dict[str, Any]:
    """meta/*.json에서 args/end_state 일부를 흡수(있으면)."""
    out: Dict[str, Any] = {}
...
def safe_read_csv(path: Path, usecols: Optional[List[str]] = None) -> Dict[str, List[float]]:
    """pandas 없이 csv만으로 일부 열을 float 리스트로 로드."""
...
def act_explore(args):
    ...
    for i in range(args.n_init):
        ...
        print(f"[explore {i+1}/{args.n_init}] cost={cost:.4g} drift={row['drift_max']:.3g} "
              f"theta_rate={row['theta_rate']:.3g} fig8={row['fig8_phase']:.3g} → {run_dir.name}")
    ...
    for it in range(args.iters):
        ...
        for j in range(n_children):
            ...
            s = space.mutate(seed, rng, sigma_frac=args.sigma_frac)
            p = Param(
                beta_dtg=s["beta_dtg"], tau_theta=s["tau_theta"], dtheta_max=s["dtheta_max"],
                theta_min=s["theta_min"], k_ctrl=s["k_ctrl"], k_ctrl_e=s["k_ctrl_e"],
                k_nu=0.0, k_nu_ed=s["k_nu_ed"], tmax=args.tmax_short,
            ...
            append_log(log_csv, row)
            evaluated.append(row)
            print(f"[mut {it+1}:{j+1}/{n_children}] cost={cost:.4g} drift={row['drift_max']:.3g} "
                  f"theta_rate={row['theta_rate']:.3g} fig8={row['fig8_phase']:.3g} → {run_dir.name}")
...
def act_exploit(args):
    ...
    for i, r in enumerate(top, 1):
        ...
        print(f"[exploit {i}/{len(top)}] cost={cost:.4g} drift={row['drift_max']:.3g} "
              f"theta_rate={row['theta_rate']:.3g} fig8={row['fig8_phase']:.3g} → {run_dir.name}")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Auto-tune DTG controller parameters (v2)")
